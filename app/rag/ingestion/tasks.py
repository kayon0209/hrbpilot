...
from app.shared.celery_app import QUEUE_INGEST, celery_app, dispatch_task
...
def dispatch_ingestion_task(task_id: str, tenant_id: str) -> None:
    # Backpressure lives inside dispatch_task: a saturated ingest queue rejects
    # new work loudly instead of accepting messages that would sit for hours.
    dispatch_task("rag.ingest", [task_id, tenant_id], QUEUE_INGEST)
