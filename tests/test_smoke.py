...
def test_create_app_boots():
    ...
    # openapi() is the authoritative source for all registered paths (incl. prefixes)
    paths = list(app.openapi().get("paths", {}).keys())
    assert len(paths) > 0
    # health router is mounted under /api
    assert "/api/health" in paths



def test_token_budget_records_usage():
    res = record_token_usage("smoke-tenant", 1000, model="gpt-4o-mini")
