...
const nav = [
  ...
  { to: '/knowledge', label: '知识库', group: '知识' }, { to: '/interview', label: '面谈纪要', group: '人才', roles: ['hrbp'] },
  { to: '/voice', label: '员工声音', group: '人才', roles: ['hrbp'] }, { to: '/weekly', label: 'HR 周报', group: '创作' },
  { to: '/culture', label: '文化内容', group: '创作' }, { to: '/evaluation', label: '检索评测', group: '系统' },
  { to: '/settings', label: '服务设置', group: '系统' },
]
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className={styles.layout}>
    <aside className={`${styles.sidebar} ${open ? styles.open : ''}`}>
      ...
      <nav aria-label="主导航">{nav.map(item => { const heading = item.group !== group ? (group = item.group) : null; const locked = !!item.roles && !item.roles.includes(user?.role ?? ''); return <div key={item.to}>{heading && <p className={styles.group}>{heading}</p>}{locked ? <span className={styles.locked} aria-disabled="true">{item.label}<small>仅 HRBP</small></span> : <NavLink to={item.to} end={item.to === '/'} onClick={() => setOpen(false)}>{item.label}</NavLink>}</div> })}</nav>
