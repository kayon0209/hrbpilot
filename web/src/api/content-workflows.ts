...
export interface WeeklySource { id: string; type: string; kind: string; label: string; created_at: string | null }
export interface WeeklyHistoryItem { report_id: string; period: string; summary: string; progress: unknown[]; risks: unknown[]; plan: unknown[]; data_sources: unknown[]; published: boolean; created_at: string | null; updated_at: string | null }
export const getWeeklySources = () => apiClient.request<{ sources: WeeklySource[]; total: number }>('/api/weekly-report/sources')
export const getWeeklyHistory = () => apiClient.request<{ reports: WeeklyHistoryItem[]; total: number }>('/api/weekly-report/history')
