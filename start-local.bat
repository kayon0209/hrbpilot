...
echo [3/4] Starting Celery worker (async tasks; -Q must match task_routes in app/shared/celery_app.py) ...
start "HRBP Celery" cmd /k ".venv\Scripts\python.exe -m celery -A app.shared.celery_app worker --loglevel=info --pool=solo --concurrency=1 -Q kb_ingest,scenario"
