...
- 状态：**已接受**（决策部分）。WP0—WP5、WP7、WP8 **已实施**（§12—§14）；
  **WP6（真实客户端认证）未做**（§14.7）—— 协议验收 10 条全过、端到端 71/71（含 CIMD 主路径、
  多租户路由、密钥轮换真实进程演练，见 §14.9），
...
- ~~**协议验收第 6 条未实现**~~ —— **已补齐**（见 §14）。`app/mcp/transport_guard.py`
  在 `/mcp` 的 ASGI 层把 scope 类拒绝翻译成 403 + `WWW-Authenticate: Bearer
...
- ~~**登录租户固定为 `"default"`，多租户部署要先解**~~ —— **已解**（见 §14.9 与 e2e 第 11 节）：
  `oauth_clients.tenant_id`（迁移 041）声明客户端归属租户，登录按客户端租户路由；
  预注册配置独占声明权，注册文档（DCR/CIMD）写了也被无视；会话与客户端租户不一致
  不得签授权码。落地时 e2e 在真实进程抓到 RLS 的 owner-bypass 让跨租户邮箱命中 ——
  修复是把过滤做进查询（`UserRepository.get_by_email(…, tenant_id=…)`），不再依赖部署细节。
- ~~**密钥轮换的执行路径没在真实进程里跑过轮换窗口**~~ —— **已演练**（见 §14.9 与 e2e 第 12 节）：
  AS 换签名密钥、旧公钥进 `OAUTH_ROTATED_PUBLIC_KEYS_PEM` 后，旧令牌仍被 RS 接受、
  新令牌经"未知 kid 强制刷新"被接受、陌生密钥仍被拒、JWKS 同时含新旧两个 kid。
- ~~**匿名/半匿名端点没有独立限流**~~ —— **已收敛为显式决定**（WP3 落地 + 本轮复核）：
  `authorize` / `token` / `introspect` 按 IP 限流（`oauth-*-ip` 桶，0 值关闭），DCR 作为
  写端点另有限速；**三个发现面（RS 元数据、AS 元数据、JWKS）维持不限流** —— 它们是
  "该去哪拿令牌"的地址簿，限流会让客户端在最脆弱的时刻（还没有任何凭据）走不下去，
  且响应是廉价的公共配置，已有 `Cache-Control` 界定新鲜度。该决定有测试守护
  （`tests/oauth/test_anonymous_endpoint_limits.py`）与 T-11/T-12 的记录。
- ~~**RS 侧元数据未设 `Cache-Control`**~~ —— **已补**：`public, max-age=60`，
  与 AS 侧元数据同一个量级（JWKS 的 300 秒仍是"撤销泄漏私钥后的最坏接受窗口"）。

## 14. 实施记录：WP3—WP8（能力补齐）

**范围**：方案 §WP3（限流、审计、撤销、敏感信息治理）、WP4（只读闭环）、WP5（WorkBuddy
连接器）、WP7（案件与审批闭环）、WP8（回滚与运维）。**WP6（真实客户端认证）未做**，见 §14.7。

### 14.1 限流（WP3）

`/mcp` 此前**完全豁免**限流。现在按 用户 / 客户端 / 安装实例 三个维度分别计数 ——
维度选错的影响不是"限流不准"，而是**看不出是谁干的**：一个失控的 Agent 会占掉为它
授权的那个人的配额，而按用户或租户排查时定位不到客户端。

维度取自 `scope["auth"]`（认证中间件解过令牌后写入），因此限流不必再解析一次令牌。
`RateLimiter` 抽出 `check_bucket(bucket, key, limit)` 支持任意维度。

OAuth 侧：`authorize` / `token` / `introspect` 按 IP 限流；`token` 额外按 `client_id`
计一维（NAT 后 IP 是共用的）。按 `client_id` 那一维放在 token 路由里而不是中间件 ——
中间件读表单会消费请求体。发现端点（元数据 / JWKS）**刻意不限流**：它被限流会让客户端
在还没有任何凭据的那一刻走不下去。429 一律带 `Retry-After`。

**已知限制**：`request.client.host` 在反向代理后面取到的是代理地址，所有用户共用一个桶。
可信代理配置写进了运维手册，不在代码里假装已解决。

### 14.2 审计（WP3）

新增 `mcp_call_audits`（迁移 039）。与 `oauth_*` 四张表相反，本表**启用并强制 RLS** ——
那四张表的查询都发生在"还不知道租户是谁"的时刻，而本表写入时租户已确定。

记录成功调用、工具层拒绝、以及**传输层 403**。第三类尤其关键：403 在 HTTP 层就返回了，
工具层的审计点根本不会执行，守卫不自己写的话这类拒绝会从审计里完全消失。

参数只存规范化 SHA-256 摘要，不存原文 —— 工具参数里可能有员工姓名、工号、薪酬区间，
审计表存原文就变成第二份（通常权限更松的）员工数据副本。摘要仍能回答"这两次调用参数
是否相同"，那正是审批重放检测需要的。

### 14.3 管理端与撤销（WP3）

新增 capability `mcp_admin`（**不**复用 `user_admin`：两者是两种不同的爆炸半径）。
四个端点：列出安装实例 / 列出客户端 / 撤销单个实例 / 撤销该客户端在本租户下的全部实例 /
全量撤销。客户端级撤销**限定本租户** —— `client_id` 是部署级对象，不限定租户会让一个
租户的管理员断掉别的租户的接入。

**已知限制**：`last_rotated_at` 是"最后一次续期"而不是"最后一次调用"（access token 是
无状态的，服务端看不到它被用的时刻）。字段名刻意不叫 `last_used_at`。

### 14.4 只读闭环与响应预算（WP4）

新增 `get_my_access_profile`（scope `hrb:profile:read`，能力位 `self_profile`，**四个
角色都持有**）。它把"为什么某个工具在我这里不见了"变成可回答的问题。

返回身份摘要、声明 scope、**有效** scope、可用工具名；**不返回**角色到能力的映射 ——
那张内部配置表泄漏出去等于把"系统里有哪些能力位"变成公开知识，而那正是越权尝试的输入。

`app/mcp/budgets.py`：三组常量（条数 5 / 单段 600 字 / 总 24000 字符）。此前限额散落
各处（`[:180]` / `[:200]` / `[:600]`），每一个都是局部决定，它们的和是多少没人知道 ——
而那个决定从来没有被谁做过。收口放在**信封层**：执行器只看得见自己那一段。

### 14.5 案件与审批闭环（WP7）

新增 `search_cases` / `get_case_summary` / `get_approval_status`。三处刻意设计：
读不到与不存在**不可区分**（否则工具成为 id 的存在性探测器）；列表**不含 `description`**
（自由文本，通常写着员工的具体情况）；审批查询**必须带 `case_id`**（只凭 approval_id
就能读，等于 id 即凭据）。

审批绑定（迁移 040）：新增 `requester_user_id` / `client_id` / `installation_id`。
`decide_approval` 会检查发起方的安装实例是否已被撤销，复用 AS/RS 共用的 `is_revoked`。
**这不是审计字段**：只挡住新调用的撤销是假的撤销 —— 那条已经躺在队列里的写操作在
审批通过时照样会落库。

### 14.6 回滚（WP8）

`MCP_EXTERNAL_ENABLED` 总开关，关掉后 `/mcp` 返回 **503 而不是 401**：401 会让客户端
启动完整的 OAuth 发现与授权，用户被拉去登录、登录完再来一次 401，而真实原因只是
"这个功能被关了"。工作台与 `/api/*` 不受影响。

跨租户紧急撤销：`scripts/revoke_all_mcp_installations.py`（默认 dry-run）。管理端 API
覆盖不了两种情况 —— 需要全公司断开的凭据泄漏，以及管理端自身不可用。

### 14.7 WP6 未做：真实客户端认证

**本轮没有完成**。方案 §WP6 要求用 WorkBuddy / Codex / Claude Code 的**真实客户端**
逐一验证，而不是用 curl 或单测推断。这需要真实账号与公网可达部署，本轮不具备。

已完成的是"服务端正确 + 一个遵守规范的标准客户端能走通"（端到端 71/71，见 §14.9）。它
**不**证明任何一个具体客户端能连上。兼容矩阵（`docs/ops/2026-09-14-client-compatibility-matrix.md`）
因此大面积标注"未实测"，并给出每一格的具体风险与验证方法。

顺带发现两处会**直接阻断** WorkBuddy 接入的差异，已处理：DCR 是必需的（默认关闭，
部署时必须打开）；WorkBuddy 优先用私有协议回调，而原实现明确拒绝自定义 scheme ——
新增 `OAUTH_CUSTOM_REDIRECT_SCHEMES` 显式清单（默认仍为空），放宽的边界由测试守住。

### 14.8 其它已知未做

- 无独立指标端点（Prometheus / OpenTelemetry），告警完全依赖日志采集；
- 无异常 DCR 的自动阻断（只有限速）；
- ~~密钥轮换未在真实进程演练~~ —— **已演练**（§14.9 与 e2e 第 12 节）；
- ~~CIMD 无端到端验证~~ —— **已验证**（单元/集成 + e2e 第 10 节），主注册路径的验证
  程度现已与 `DCR` 持平，见兼容矩阵 `CIMD 客户端身份` 行；
- ~~登录租户固定为 `"default"`，多租户要先解~~ —— **已解**（§14.9 与 e2e 第 11 节）。

### 14.9 实施记录：多租户路由、匿名端点收尾、轮换演练（收尾轮）

**范围**：§13.6 剩余四项中可本地落地的全部三项，外加上一轮遗留的 mypy 类型修正。
全部在本地 scratch 库 `hrbp_oauth_e2e` 以**真实进程**验证，端到端 **71/71，退出码 0**。

**多租户登录路由（客户端 → 租户）**

- `oauth_clients` 新增 `tenant_id`（迁移 `041_oauth_client_tenant`，NOT NULL，
  `server_default='default'`，升级/降级往返实测）。这张表刻意不上 RLS，该列不承担
  隔离职责，只回答"授权这个客户端时去哪个用户目录找人"。
- 声明权归**预注册配置**独占（条目新增 `tenant_id` 键；写空/null 让启动失败，避免
  笔误静默落回缺省租户）。DCR / CIMD 的注册文档即使带 `tenant_id` 也被无视 ——
  让自助注册自选租户等于把跨租户入口交给匿名请求。
- 授权页与同意页各有一道租户闸：会话租户 ≠ 客户端租户 → 回登录页。没有这道闸，
  一个在缺省租户登录的会话就能给 acme 客户端签授权码，令牌上的租户成了表单可改的字段。
- **e2e 抓到的真 bug**：首轮 71 项里两个负例失败 —— 缺省租户的邮箱在 acme 客户端的
  登录中命中了。根因是 `users` 的 RLS 刻意不 FORCE（迁移 014：登录发生在租户上下文
  之前），而"角色不是表 owner"只是一个碰巧成立的部署细节；scratch 库把 owner 转给
  应用角色后 owner-bypass 立即现形。修复：`UserRepository.get_by_email` 增加**显式**
  `tenant_id` 过滤，登录正确性从此不依赖 RLS 的任何配置取值。RLS 与过滤同时存在的
  意义是纵深防御，不是二选一。

**匿名端点（T-11/T-12）**

- 内省限流 WP3 已落地（`oauth-introspect-ip`），威胁模型里"无限流"的缺口文字过时；
- RS 元数据补 `Cache-Control: public, max-age=60`（与 AS 同量级）；
- 三个发现面维持**不限流**的显式决定（理由见 §13.6），由测试守护。

**密钥轮换真实进程演练**

- 轮换动作本体进 e2e 第 12 节：停 AS → 以新私钥 + 旧公钥（rotated）在同一端口重启
  （issuer 不变，正是部署里轮换的真实形态）。断言五条：JWKS 含新旧两个 kid、旧令牌
  仍被接受（轮换窗口的用户无感）、新令牌经 RS 的"未知 kid 强制刷新一次"被接受、
  陌生密钥仍 401、全链路令牌照常签发。
